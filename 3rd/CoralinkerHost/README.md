...
│  - 会话控制: Start, Stop                                         │
│  - 数据管理: GetAllCartFields, SetCartField                      │
...
| Store | 职责 |
|-------|------|
| `project` | selectedAsset, selectedFile, lastBuildId（不存节点） |
| `runtime` | nodeStates, nodeInfos, variables, appState |
| `files` | 文件树、打开的 Tab |
| `logs` | 终端日志、节点日志（seq 分页） |
| `ui` | 通知、对话框 |
...
- **前端**: Vue 3, Vite, TypeScript, Pinia, Vue-flow, Naive UI
